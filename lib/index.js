  ...
  var reassignmentVisitor = {
    ...
    AssignmentExpression: function AssignmentExpression(path) {
      ...
      for (var _iterator = exports, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _getIterator(_iterator);;) {
        ...
        node = buildExportsAssignment(t.stringLiteral(reid.name), node).expression;
      }
        ...
        exit: function exit(path) {
          ...
          for (var _i3 = 0; _i3 < body.length; _i3++) {
            ...
            if (_path.isImportDeclaration()) {
              ...
              if (declaration.isFunctionDeclaration()) {
                ...
                var defNode = t.stringLiteral("default");
                if (id) {
                ...
                _path.replaceWith(buildExportsAssignment(t.stringLiteral("default"), declaration.node));
              ...
              if (declaration.node) {
                if (declaration.isFunctionDeclaration()) {
                  ...
                  topNodes.push(buildExportsAssignment(t.stringLiteral(id.name), id));
                  _path.replaceWith(declaration.node);
                } else if (declaration.isClassDeclaration()) {
                  var id = declaration.node.id;
                  addTo(exports, id.name, id);
                  _path.replaceWithMultiple([declaration.node, buildExportsAssignment(t.stringLiteral(id.name), id)]);
                  ...
                  for (var _iterator5 = declarators, _isArray5 = Array.isArray(_iterator5), _i6 = 0, _iterator5 = _isArray5 ? _iterator5 : _getIterator(_iterator5);;) {
                    ...
                    if (id.isIdentifier()) {
                      ...
                      init.replaceWith(buildExportsAssignment(t.stringLiteral(id.node.name), init.node).expression);
                ...
                if (source) {
                  ...
                  for (var _iterator7 = specifiers, _isArray7 = Array.isArray(_iterator7), _i8 = 0, _iterator7 = _isArray7 ? _iterator7 : _getIterator(_iterator7);;) {
                    ...
                    if (specifier.isExportSpecifier()) {
                      ...
                      nonHoistedExportNames[specifier.node.exported.name] = true;
                      nodes.push(buildExportsAssignment(t.stringLiteral(specifier.node.exported.name), specifier.node.local));
                    }
                  }
                }
          ...
          if (hasImports && _Object$keys(nonHoistedExportNames).length) {
            ...
            for (var _name in nonHoistedExportNames) {
              hoistedExportsNode = buildExportsAssignment(t.stringLiteral(_name), hoistedExportsNode).expression;
            }
